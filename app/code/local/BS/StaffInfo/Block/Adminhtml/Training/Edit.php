 ...
 */
/**
 * Related Training admin edit form
 *
 * @category    BS
 * @package     BS_StaffInfo
 * @author Bui Phong
 */
class BS_StaffInfo_Block_Adminhtml_Training_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * constructor
     *
    ...
    public function getHeaderText()
    {
        if (Mage::registry('current_training') && Mage::registry('current_training')->getId()) {
            return Mage::helper('bs_staffinfo')->__(
                "Edit Related Training '%s'",
                $this->escapeHtml(Mage::registry('current_training')->getCourse())
            );
        } else {
            return Mage::helper('bs_staffinfo')->__('Add Related Training');
        }
    }
}
