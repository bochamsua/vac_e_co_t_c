 ...
 */
/**
 * Docwise Document admin edit form
 *
 * @category    BS
 * @package     BS_Docwise
 * @author Bui Phong
 */
class BS_Docwise_Block_Adminhtml_Docwisement_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * constructor
     *
    ...
    {
        if (Mage::registry('current_docwisement') && Mage::registry('current_docwisement')->getId()) {
            return Mage::helper('bs_docwise')->__(
                "Edit Docwise Document '%s'",
                $this->escapeHtml(Mage::registry('current_docwisement')->getDocName())
            );
        } else {
            return Mage::helper('bs_docwise')->__('Add Docwise Document');
        }
    }
}
