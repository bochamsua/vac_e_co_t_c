 ...
 */
/**
 * Score admin edit form
 *
 * @category    BS
 * @package     BS_Docwise
 * @author Bui Phong
 */
class BS_Docwise_Block_Adminhtml_Score_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * constructor
     *
    ...
    {
        if (Mage::registry('current_score') && Mage::registry('current_score')->getId()) {
            return Mage::helper('bs_docwise')->__(
                "Edit Score '%s'",
                $this->escapeHtml(Mage::registry('current_score')->getTraineeName())
            );
        } else {
            return Mage::helper('bs_docwise')->__('Add Score');
        }
    }
}
