 ...
 */
/**
 * TC Task admin edit form
 *
 * @category    BS
 * @package     BS_Report
 * @author Bui Phong
 */
class BS_Report_Block_Adminhtml_Tctask_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * constructor
     *
    ...
    {
        if (Mage::registry('current_tctask') && Mage::registry('current_tctask')->getId()) {
            return Mage::helper('bs_report')->__(
                "Edit TC Task '%s'",
                $this->escapeHtml(Mage::registry('current_tctask')->getTctaskName())
            );
        } else {
            return Mage::helper('bs_report')->__('Add TC Task');
        }
    }
}
