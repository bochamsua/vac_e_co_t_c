 ...
 */
/**
 * Worksheet Document admin edit form
 *
 * @category    BS
 * @package     BS_WorksheetDoc
 * @author      Bui Phong
 */
class BS_WorksheetDoc_Block_Adminhtml_Worksheetdoc_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * constructor
     *
    ...
    {
        if (Mage::registry('current_worksheetdoc') && Mage::registry('current_worksheetdoc')->getId()) {
            return Mage::helper('bs_worksheetdoc')->__(
                "Edit Worksheet Document '%s'",
                $this->escapeHtml(Mage::registry('current_worksheetdoc')->getWsdocName())
            );
        } else {
            return Mage::helper('bs_worksheetdoc')->__('Add Worksheet Document');
        }
    }
}
