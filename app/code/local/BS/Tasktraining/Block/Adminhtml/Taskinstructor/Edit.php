 ...
 * @author Bui Phong
 */
class BS_Tasktraining_Block_Adminhtml_Taskinstructor_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * constructor
     *
    ...
    {
        if (Mage::registry('current_taskinstructor') && Mage::registry('current_taskinstructor')->getId()) {
            return Mage::helper('bs_tasktraining')->__(
                "Edit Instructor '%s'",
                $this->escapeHtml(Mage::registry('current_taskinstructor')->getName())
            );
        } else {
            return Mage::helper('bs_tasktraining')->__('Add Instructor');
        }
    }
