...
$curriculum    = Mage::getModel('bs_traininglist/curriculum')
    ->getCollection()
    ->addAttributeToFilter('c_code', array('in'=>array('AMB-UA-B12', 'AMB-S1-B12', 'AMB-S2-B12')))
;
...
foreach ($curriculum as $c) {
    ...
    if($subjects->count()){
        ...
        foreach ($subjects as $sub) {
            ...
            //save position
            $sub->setSubjectOrder($i)->save();

            $examHour = getExamHour($hour);
            if($examHour){
                // add Exam as subject
                $subject = Mage::getModel('bs_subject/subject');
                $subject->setCurriculumId($c->getId());
                $subject->setSubjectName('Examination - '.$code);
                $subject->setSubjectCode(generateRandomString().'-'.$i);
                $subject->setSubjectHour($examHour);
                $subject->setSubjectShortcode($code);
                $subject->setSubjectExam(1);
                $subject->setSubjectOrder($i+5);
                $subject->save();
            }
