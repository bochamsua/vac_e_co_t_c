...
$taskIn = Mage::getModel('bs_tasktraining/taskinstructor')->getCollection();
if($taskIn->count()){
    $taskIn->walk('delete');
}
...
if(count($list)){
    ...
    foreach ($list as $item) {
        $vaecoIds[$item['vaeco_id']] = 1;


    }

    $vaecoIds = array_keys($vaecoIds);
    foreach ($vaecoIds as $vaecoId) {
        ...
        if($cus->getId()){
            ...
            $taskInstructor->setName($name)->setVaecoId($vaecoId);
            $res = $taskInstructor->save();
            if($res){
                echo "Done ".$vaecoId."\n<br>";
            }
        }
