...
if(count($list)){
    ...
    foreach ($list as $item) {
        ...
        $found = "http://portal.vaeco.com.vn/portal/viewuser.asp?username=".$username;
        ...
        $otherInfo = $readConnection->fetchRow("SELECT * FROM bstaff WHERE StaffID = '{$id}'");
        $sex = null;
        $birthday = null;
        $phoneNumber = null;
        $jointDate = null;
        ...
        if($otherInfo){
            $sex = $otherInfo['Sex'];
            $birthday = $otherInfo['BirthDay'];
            $phoneNumber = $otherInfo['PhoneNumber'];
            $jointDate = $otherInfo['JoinDate'];
        ...
        if($id != 'VAE00000'){
            $run = $writeConnection->query("INSERT INTO bstaffs VALUES (null,'{$user}','{$id}','{$name}','{$phone}','{$position}','{$division}','{$department}','{$sex}','{$birthday}','{$jointDate}',null)");
            if($run){
                echo "Done {$name} <br>";
            }
        }
